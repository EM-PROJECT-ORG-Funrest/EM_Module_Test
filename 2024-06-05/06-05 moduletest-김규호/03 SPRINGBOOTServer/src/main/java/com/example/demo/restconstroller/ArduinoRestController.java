...
public class ArduinoRestController {
    ...
    public ResponseEntity<String> setConnection(@PathVariable("COM") String COM, HttpServletRequest request) { //url 경로에 있는 COM 가져와서 사용
        log.info("GET /arduino/connection " + COM + " IP: " + request.getRemoteAddr()); // getRemoteAddr() 사용자 ip 추출

        if (serialPort != null) { // serialProt가 null 이아니면 port를 닫고 새 port 생성
        ...
        if (isOpen) {
            ...
            return new ResponseEntity("Connection Success!", HttpStatus.OK); // HttpStatus 값에 대한 반환
        } else {
    ...
    public void led_Control(@PathVariable String value, HttpServletRequest request) throws IOException { //url에 value값 가져오기
